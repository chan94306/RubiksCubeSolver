...
//	private boolean captureButton = false, continueButton = false;
	...
	public DrawOnTop(Context context, Cube current) {
		...
		avgPaint = new Paint();
		avgPaint.setStyle(Paint.Style.FILL);			/// this is the one and only
		avgPaint.setColor(Color.RED);
		avgPaint.setTextSize(25);
	...
	public void readFace(int face){
		for(int i = 0; i < 3; i++){
			for(int j = 0; j < 3; j++){
				current.setSquare(face, i, j, getIdealColor(paintGrid[i][j].getColor()));
			}
		}
		current.setColorValue(face, getIdealColor(paintGrid[1][1].getColor()));
		//		Log.e("ideal color for face: " + face, "" + getIdealColor(paintGrid[1][1].getColor()));
	}
	...
	// Either assign color values first, and then comb though and turn them into 0-5, 
...
//				Log.e("ideal", "ORANGE_0xFF8800");
				return 0xFF8800;					// ORANGE, this is legal usage after checking that it's correctly interpreted as a hex number
			}
...
//				Log.e("ideal", "BLACK- something went wrong");
	...
	// Turns the semi-raw Color values + orange into integers 0 to 5, the final step before solving
	@SuppressLint("UseSparseArrays")
...
//	}
	...
	public void drawGrid(Canvas canvas){
		/**
		 * @TODO xyzz - reorganize this into method, probably in the main activity class, because it's used multiple times
		 */
...
//		}
		...
		canvas.drawLine(leftBound, topBound+2*squareLength, leftBound+3*squareLength, topBound+2*squareLength, mPaintRed);
		canvas.drawLine(leftBound+1*squareLength, topBound, leftBound+1*squareLength, topBound+3*squareLength, mPaintRed);
		canvas.drawLine(leftBound+2*squareLength, topBound, leftBound+2*squareLength, topBound+3*squareLength, mPaintRed);


		// Herp derp change this later
		for(int i = 0; i < paintGrid.length; i++){
			for(int j = 0; j < paintGrid[i].length; j++){
				canvas.drawCircle(leftBound + squareLength*j + squareLength/2, topBound + squareLength*i + squareLength/2, squareLength/2, paintGrid[i][j]);
			}
		}
	}
...
//				Log.e("DOT 12/31", ""+ leftMargin + " " + topMargin + " " + topMargin + " " + topMargin);
				for (int j = topBound + MARGIN; j < topBound + squareLength - MARGIN; j++) {					
					int pix = j*mImageWidth + leftBound + MARGIN;								// check border conditions !A!!A!
					for (int i = leftBound + MARGIN; i < leftBound + squareLength - MARGIN; i++, pix++) {
//						Log.e("DOT 12/31", "" + pix);
				...
				// Number of pixels traversed should be the area of the square, but make sure to double check
